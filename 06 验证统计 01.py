...
jieba.load_userdict("data/add_words.txt")
...
class BayesSingleDataConductor(object):
    ...
    def load_add_words():
        """ 加载并添加自定义词库 """
        # 加载白词表
        add_words_filepath = "data/add_words_jieba.txt"
        jieba.load_userdict(add_words_filepath)
    ...
    def cut_and_wash_words(cls, contents, stop_word):
        ...
        for content in tqdm.tqdm(contents, desc="邮件分词中：", colour="yellow"):
            ...
            # jieba 分词：精确模式
            words = list(jieba.cut(content, cut_all=False, HMM=True))
            # words = list(jieba.cut(content, cut_all=False, HMM=True))
            # 去除停用词
