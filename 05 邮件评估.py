...
class EmailsJudgement(object):
    ...
    @staticmethod
    def emails_evaluation(p_value, file_name):
        """ 邮件判别 """
        # 判别邮件是否为垃圾邮件
        if p_value > 0.9:
            print("*" * 50)
            print(f"名称为 {file_name} 的邮件属于 \033[1;31m垃圾邮件\033[0m")
            print("*" * 50)
            print()
            return True
        else:
            print("*" * 50)
            print(f"名称为 {file_name} 的邮件属于 \033[1;32m正常邮件\033[0m")
            print("*" * 50)
            print()
            return False
...
def main():
    ...
    # 统计测试邮件词频
    contents_test, file_name = BayesTraverseFiles.traverse_test_files(test_emails)
    ...
    # 计算测试邮件词频的联合概率
    p_value = BayesAlgorithm.bayes_calculation(test_words_prob)

    # 邮件判别
    EmailsJudgement.emails_evaluation(p_value, file_name)

    # 邮件内容复查
    email_content_check(p_value, file_name)
